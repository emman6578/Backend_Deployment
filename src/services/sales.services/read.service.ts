...
const calculateSalesSummary = (sales: any[]): SalesSummary => {
  const summary = sales.reduce(
    (acc, sale) => {
      // If unitFinalPrice is already the total price for this sale
      const revenue = sale.unitFinalPrice; // Remove quantity multiplication

      // COGS calculation remains the same
      const cogs = sale.inventoryItem?.costPrice
        ? sale.quantity * sale.inventoryItem.costPrice
        : 0;

      acc.totalRevenue += Number(revenue); // Ensure it's a number
